...
<!--<div class="modal fade" id="team_s" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <div class="card border-0 shadow-sm">
           <div class="card-body">
            <div class="d-flex align-items-center justify-content-between mb-3">
              <h5 class="card-title m-0">FOOTER</h5>
              <button type="button" class="btn btn-dark shadow-none btn-sm">
              <a href="add_footer.php" class="text-white text-decoration-none">
  ...
  <?php
    if(Connect()){      
                  ...
                  $start = ($page - 1) * $noOfRows;
                     $sql = "SELECT * FROM `footer` limit $start, $noOfRows";
                     $rs = dbQuery($sql);
    ...
    <tr>
      <td><?= $id ?></td>
      <td><?= $name ?></td>
      <td><a href="edit_footer.php?id=<?= $row['id'] ?>"><i class="bi bi-pencil-square text-white"></i></a> | 
                    <a href="delete_footer.php?id=<?= $row['id'] ?>" onclick="return confirm('Are You Sure To Delete This Record!')">
